...
var require_core = __commonJS({
  "node_modules/ajv/dist/core.js"(exports) {
    ...
    function addInitialKeywords(defs2) {
      ...
      for (const keyword in defs2) {
        const def = defs2[keyword];
        if (!def.keyword)
          def.keyword = keyword;
        this.addKeyword(def);
      }
...
function parseFrontmatter(raw) {
  ...
  const lineOffset = 1;
  const lc = new import_yaml.LineCounter();
  const doc = (0, import_yaml.parseDocument)(fmText, { lineCounter: lc });
  let parseError;
  let parseErrorRange;
  ...
  const valueRange = (key) => {
    if (!(0, import_yaml.isMap)(doc.contents)) return void 0;
    for (const item of doc.contents.items) {
      if (!(0, import_yaml.isScalar)(item.key) || item.key.value !== key) continue;
      const node = (0, import_yaml.isScalar)(item.value) ? item.value : item.key;
...
function pluginManifestValidator() {
  if (!cachedValidator) {
    cachedValidator = new import__3.Ajv2020({ allErrors: true, strict: false }).compile(plugin_schema_default);
  }
...
function reportText(diagnostics, dialectIds) {
  ...
  for (const d of diagnostics) {
    if (d.file !== file) {
      file = d.file;
      out.push(import_picocolors.default.underline(file));
    }
    const mark = d.severity === "error" ? import_picocolors.default.red("\u2716") : import_picocolors.default.yellow("\u26A0");
    const tags = import_picocolors.default.dim(`[${d.dialects.join(", ")}]`);
    const loc = d.range ? import_picocolors.default.dim(` (${d.range.start.line}:${d.range.start.col})`) : "";
    out.push(`  ${mark} ${tags} ${d.rule}  ${d.message}${loc}`);
  }
  if (diagnostics.length > 0) out.push("");
  const errors = diagnostics.filter((d) => d.severity === "error").length;
  const warnings = diagnostics.length - errors;
  const summary = diagnostics.length === 0 ? import_picocolors.default.green("\u2713 no problems found") : `${errors > 0 ? import_picocolors.default.red(`${errors} error${errors === 1 ? "" : "s"}`) : "0 errors"} \xB7 ${warnings > 0 ? import_picocolors.default.yellow(`${warnings} warning${warnings === 1 ? "" : "s"}`) : "0 warnings"}`;
  out.push(summary);
  return out.join("\n");
}
